...
**Every entry gets a handle: its bucket letter plus its position in that bucket** — `B1`, `B2`, `D1`, `C1`, `A5`, numbered in printed order, restarting at 1 per bucket. It is how the user names an entry back ("fix B2 and A5"), so put it at the head of every entry, cite it whenever the report points at one (the summary line, a cross-reference), and carry the same handles into any follow-up. If a later message re-lists entries, keep the handles they were given rather than renumbering. In the artifact the handles are generated by a CSS counter — see `report-template.md`, and never type one in there by hand.
